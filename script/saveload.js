...
function load(){
	...
	for(let i=1;i<=game.annihilation.challenged.length;i++){
		if(game.annihilation.challenged[i-1]>=1){
		...
		if(game.annihilation.inchallenge[i-1]==1){
	...
	if(game.annihilation.hasannihilated){
		...
		document.getElementById("annihilation_vertex_tab").style.display='inline';
	}
	...
	if(game.infinity.challenged.indexOf(0) === -1||game.eternity.haseternitied) document.getElementById("ordinal_tab").style.display='inline';
	if(game.eternity.milestones[9]==1) document.getElementById("eternity_generator_tab").style.display='inline';
	if(game.eternity.milestones[10]==1) document.getElementById("eternity_upgrade_tab").style.display='inline';
	if(game.eternity.milestones[15]==1) document.getElementById("eternity_producer_tab").style.display='inline';
	...
	if(game.time!=0) {
		...
		setTimeout(function(){alert("你已经连续玩了很久了，建议手动存个档");},3600000);
		setTimeout(function(){alert("你真的已经连续玩了很久了，建议手动存个档，然后休息一下");},14400000);
	}
